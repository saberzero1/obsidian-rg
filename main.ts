...
export default class ObsidianRg extends Plugin {
  ...
  async onload() {
    ...
    this.rootPath = (this.app.vault.adapter as FileSystemAdapter).getBasePath();
  }
...
class ObsidianRgModal extends Modal {
  ...
  async childCall(query: string): Promise<RgResult> {
    const { stdout } = await this.exec_promise(
      `${this.plugin.settings.rgLocation} "${query}" "${this.plugin.rootPath}" --json`,
      { signal: this.signal },
    );
    return stdout
      .trimRight()
      .split("\n")
      .map(JSON.parse)
      .filter((res: { type: string }) => res.type === "match");
  }

  updateNode(parentElem: HTMLElement, div: HTMLElement) {
    const resultContainer = Array.from(parentElem.children).find((elem) =>
      elem.classList.contains("prompt-results"),
    );
    if (resultContainer) {
  ...
  resultAsHTML(rgResult: RgResult): HTMLElement[] {
    return rgResult.map(({ data }) => {
      const path = data.path.text.replace(this.plugin.rootPath + "/", "");
  ...
  onOpen() {
    ...
    input.onkeydown = debounce(
      (e: KeyboardEvent) => this.search(e, contentEl),
      300,
      false,
    );
